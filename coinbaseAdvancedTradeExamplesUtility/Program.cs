...
namespace CoinbaseAdvancedTrade.Utility
{
    class Program 
    {
        ...
        static async Task Main(string[] args) {
            ...
            System.Console.WriteLine("Enter Order ID to get info: ");
            string orderID = "";
            orderID = Console.ReadLine();
            var orderHistoricalResponse = await CoinBase.OrdersService.GetOrderByIdAsync(orderID);
            foreach(PropertyDescriptor descriptor in TypeDescriptor.GetProperties(orderHistoricalResponse.order))
            {
                string name = descriptor.Name;
                object value = descriptor.GetValue(orderHistoricalResponse.order);
